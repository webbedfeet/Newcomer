...
blah=as_data_frame(apply(t(apply(as_data_frame(-study_dat),1, rank, ties.method='min')),2, table))
blah$rank = 1:9
blah %>% gather(variable, value, -rank) %>% ggplot(aes(factor(variable), value))+geom_bar(stat='identity', aes(fill=factor(rank)))

ind <- apply(study_dat,1, function(x) all(x>95))
